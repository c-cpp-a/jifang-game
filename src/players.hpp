...
//		mp["cyan"]=0x0B;
		...
		cerr << card_names[cards[cardid].first];
		if(raritynames[cards[cardid].second].size()){
			cerr << '[' << raritynames[cards[cardid].second] << ']';
		}
...
struct Group{
	...
	void gameinfo(player &nowplayer){
		...
		for(int i=1;i<=playersum;i++){
			...
			cerr << players[i].name;
			if(players[i].spriteid)	cerr << "[" << sprite_names[players[i].spriteid] << "]";
			cerr << "\t生命 " << players[i].life << "/20\t成绩 " << players[i].score << "/400\t知识点 " << players[i].knowledge << "/40";
			if(players[i].jcturn()){
				cerr << "[机惨还剩 " << players[i].jcturn() << " 回合]";
			}
			cerr << "\n";
		}
		cerr << "玩家：" << nowplayer.name << "的回合";
		if(nowplayer.isAI){
			cerr << "[AI代管]";
		}
		cerr << "\n";
	...
	void usecard(int cardid,int playerid){
		...
		cerr << players[playerid].name << "使用了" << card_names[_usecard] << "\n";
		...
		case 1:
			{
				...
				cerr << players[playerid].name << "成绩+" << addsc << " 知识点+" << addkng << "\n";
				players[playerid].add_sc(addsc);
				players[playerid].add_kng(addkng);
				if(spritename=="xza"){
					cerr << "xza发动了被动技能！学新知识点，成绩再+2。\n";
		...
		case 2:
			{
				...
				if(spritename=="xza"){
					cerr << "xza发动了被动技能[蓝勾爷]！做题时各效果再x1.6。\n";
		...
		case 3:
			{
				...
				addsc=ceil(addsc*rarity_times[_rarity]);
				if(spritename=="xza"){
					cerr << players[playerid].name << "发动了被动技能[蓝勾爷]！狂人卷题时各效果再x1.6。\n";
					addsc=ceil(addsc*1.6);
					addlf=ceil(-addlf)*-1.6;
				} 
				cerr << players[playerid].name << "生命-" << -addlf << " 成绩+" << addsc << "\n";
				players[playerid].add_sc(addsc);
		...
		case 5:
			{
				...
				if(spritename=="lyr"){
					addlf++;
					addsc-=2;
					cerr << players[playerid].name << "发动了被动技能！使用卡牌5时生命再+1，成绩再-2。\n";
					...
					if(randint(1,10)<=4){
						cerr << players[playerid].name << "发动了被动技能！成绩再-7。\n";
						addsc-=7;
					}
				} else if(spritename=="nmk"){
					cerr << players[playerid].name << "发动了被动技能[腐败小助手]！清空本轮腐败效果。\n";
					players[playerid].tags["corruption"]="false";
				}
				players[playerid].add_lf(addlf);
				players[playerid].add_sc(addsc);
		...
		case 6:
			{
				...
				auto f=[](vector<player> &players,const string &spritename,int playerid,int &addsc_self,int &addlf_self){
					if(spritename=="xza"){
						cerr << players[playerid].name << "发动了被动技能！成为卡牌6的目标时成绩改为减去1/4（向下取整）。\n";
				...
				cerr << players[playerid].name << "生命+" << addlf_self << " 成绩-" << -addsc_self << "\n";
				cerr << players[chooseplayer].name << "生命+" << addlf_it << " 成绩-" << -addsc_it << "\n";
		...
		case 7:
			{
				...
				if(spritename=="AJ"){
					cerr << "发动被动技能[抓腐]！使用卡牌7对所有人（自己除外）有效。\n";
					for(int i=1;i<=playersum;i++){
						if(i==playerid)	continue;
						if(players[i].tags["corruption"]=="true"){
							cerr << players[i].name << "在腐败。生命-" << -addlf2 << "。\n";
							players[i].add_lf(addlf2);
						} else{
							cerr << players[i].name << "没有腐败。生命-" << -addlf1 << "。\n";
				...
				if(sprite_names[players[chooseplayer].spriteid]=="AJ"){
					...
					cerr << "AJ发动被动技能！向AJ举报AJ的人生命-" << -addlf << "，AJ不受影响。\n";
					players[playerid].add_lf(addlf);
					break;
				}
				int addlf=addlf1;
				...
				cerr << players[chooseplayer].name << "生命-" << -addlf << "\n";
			}
			break;
		case 8:
			{
				...
				if(spritename=="cyq"){
					...
					cerr << players[playerid].name << "发动了被动技能[急眼哥]！使用卡牌8时改为所有其它角色生命-2，自己成绩改为-7；如果角色腐败过，成绩-" << -addsc << "。\n";
					cerr << players[playerid].name << "成绩-7\n";
					players[playerid].add_sc(-7);
					for(int i=1;i<=playersum;i++){
						if(i==playerid || players[playerid].isdead)	continue;
						cerr << players[i].name << "生命-2 ";
						players[i].add_lf(-2);
						if(players[i].tags["corruption"]=="true"){
							cerr << "成绩-" << -addsc;
				...
				cerr << players[playerid].name << "成绩-" << -addsc_self << "\n";
				cerr << players[chooseplayer].name << "生命-" << -addlf_it;
				if(addsc_it){
					cerr << " 成绩-" << -addsc_it;
				}
				cerr << "\n";
		...
		case 9:
			{
				for(int i=1;i<=playersum;i++){
					...
					if(players[i].tags["involution"]=="true"){
						cerr << players[i].name << "在内卷\n";
					} else{
						int addsc=-ceil(3.5*rarity_times[_rarity]),addlf=-1;
						cerr << players[i].name << "成绩-" << -addsc << " 生命-" << -addlf << "\n";
...
//						cerr << card_names[card.first];
//						if(raritynames[card.second].size()){
//							cerr << '[' << raritynames[card.second] << ']';
//						}
//						SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x07);
	...
	void startskill(int i){
		...
		for(int j=0;j<skill_cnt;j++){
			cerr << j+1 << ' ' << skill_names[players[i].spriteid][j] << '\t';
		}
		cerr << '\n';
		if(players[i].isAI){
			Sleep(AI_THINK_MS);
		...
		if(op!='0'){
			op=op-'0'-1;
			cerr << players[i].name << "发动了[主动技]" << skill_names[players[i].spriteid][op] << "。\n";
			switch(players[i].spriteid){
			case 1:
				if(op==0){
					//腐败王
					cerr << "视为使用一张[腐败]，本轮所有角色无法使用[学新知识点]。\n";
					int addlf=1,addsc=-4.2;
					cerr << players[i].name << "生命+" << addlf << " 成绩-" << -addsc << "\n";
					addlf++;
					addsc-=2;
					cerr << players[i].name << "发动了被动技能！使用卡牌5时生命再+1，成绩再-2。\n";
					players[i].add_lf(addlf);
					players[i].add_sc(addsc);
					...
					//制裁
					cerr << "自减5点生命，其他角色本轮卡牌[禁止内卷][腐败][一起腐败]失效。\n";
					cerr << players[i].name << "生命-5。\n";
					players[i].add_lf(-5);
					for(int j=1;j<=playersum;j++){
						if(j==i)	continue;
			...
			case 3:
				if(op==0){
					//出题人
					cerr << "（视作打出[做题]）所有人的成绩加上自己的知识点数量乘以0.1，自己成绩+5。\n";
					cerr << players[i].name << "成绩+5\n";
					players[i].add_sc(5);
					for(int j=1;j<=playersum;j++){
						int addsc=ceil(players[j].knowledge*0.1);
						if(sprite_names[players[j].spriteid]=="xza"){
							cerr << players[j].name << "发动了被动技能[蓝勾爷]！效果结算后*1.6。\n";
							addsc=ceil(addsc*1.6);
						}
			...
			case 4:
				if(op==0){
					//水谷
					cerr << "本轮无法出[禁止内卷]。已有的[禁止内卷]无效（卡牌[学新知识点][做题][狂人卷题]有效），无视技能。\n";
					for(int j=1;j<=playersum;j++){
	...
	void popcard(int i){
		...
		while(players[i].cards.size()>6){
			system("cls");
			cerr << "您还需要弃掉 " << players[i].cards.size()-6 << " 张牌。";
			for(int j=0,siz=players[i].cards.size();j<siz;j++){
//				pair<int,int> card=players[i].cards[j];
//				cerr << j+1 << ' ' << card_names[card.first];
//				if(card.second>1){
//					cerr << '[' << raritynames[card.second] << ']';
//				}
				players[i].showcard(j);
